...
const selectCardImages = chooseN(cardImages, 6); //0905 6->5
console.log(selectCardImages);

var whichESGFile = String((Math.floor(Math.random()*72968)+1)%2);
function decideFrontFaceImg() {
  for (let i=0; i<selectCardImages.length; i++) { //0905 0->1, i<s... => i<=
    frontFaces[2*i].src = './img/Question/' + selectCardImages[i]; 
    frontFaces[2*i+1].src = './img/ANS/'+ selectCardImages[i];
    //frontFaces[2*i].src = './img/ESG' + whichESGFile + '/' + selectCardImages[i]; //0905 i->i-1
    //frontFaces[2*i+1].src = './img/ESG' + whichESGFile + 'Ans/'+ selectCardImages[i];
    // backFaces[2*i].src   = './img/mosquito/CardBackground.png';
    // backFaces[2*i+1].src = './img/mosquito/CardBackground.4.png';
  }
  //frontFaces[0].src  = './img/ESG' + whichESGFile + '/' + selectCardImages[selectCardImages.length-1];
  //frontFaces[1].src = './img/ESG' + whichESGFile + 'Ans/' + selectCardImages[selectCardImages.length-1];
